...
{
  public static StringBuffer generateReport(PrintWriter out, StringBuffer sbuf, Map parameterValues, String reportText, Session dataSession)
  {
  ...
  private static int generateReport(PrintWriter out, StringBuffer sbuf, Map parameterValues, String reportText, int curPos, Connection conn)
  {
  ...
  private static int query(PrintWriter out, StringBuffer sbuf, Map parameterValues, Connection conn, String reportText, String sqlSource, int curPos)
  {
    String command = "";
      ...
      } else
      {
        ...
        while (true)
        {
          while (rset.next())
          {
            ...
            for (int i = 1; i <= columnCount; i++)
            {
              parameterValues.put(meta.getColumnLabel(i).toUpperCase(), cleanNull(rset.getString(i)));
            }
            if (command.equals(COMMAND_LOOP))
            {
  ...
  private static void performGET(PrintWriter out, StringBuffer sbuf, ResultSet rset) throws SQLException
  {
    if (rset.next())
    {
  ...
  private static PreparedStatement prepareStatement(Map parameterValues, Connection conn, int sqlListPos, String[] sqlList) throws SQLException
  {
  ...
  private static int print(PrintWriter out, StringBuffer sbuf, Map parameterValues, String reportText, int curPos, int endPos)
  {
    if (parameterValues != null)
    {
  ...
  private static String removeReportTag(String reportText)
  {
    ...
    {
      throw new IllegalArgumentException("Could not find properly formatted <report> tag in report defintion text");
    }
    posStart++;
