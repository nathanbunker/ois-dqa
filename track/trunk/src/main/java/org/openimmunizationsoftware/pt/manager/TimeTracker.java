...
public class TimeTracker
{
  ...
  {
    if (billEntry != null && billEntry.getBillable() != null && billEntry.getBillable().equals("Y"))
    {
  ...
  private synchronized void init(WebUser webUser, Session dataSession, Calendar t)
  {
    ...
    startDate = today;
    endDate = tomorrow;
    Query query = dataSession.createQuery("from BillEntry where username = ? and startTime >= ? and startTime < ?");
  ...
  private synchronized void init(WebUser webUser, Session dataSession, Calendar t, int calendarField)
  {
    totalMinsForProjectMap = new HashMap<Integer, Integer>();
    totalMinsForClientMap = new HashMap<String, Integer>();
    totalMinsForBillCodeMap = new HashMap<String, Integer>();
  ...
  public static int readTime(String timeString)
  {
    ...
    if (pos != -1)
    {
      min = Integer.parseInt(timeString.substring(0, pos)) * 60 + Integer.parseInt(timeString.substring(pos + 1));
    } else
    {
  ...
  {
    return billCode.getBillRound() * (int) ((min + billCode.getBillRound() / 2) / billCode.getBillRound());
  }
}
