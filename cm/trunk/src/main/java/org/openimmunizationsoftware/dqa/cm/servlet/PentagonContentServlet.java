  ...
  protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException
  {
    ...
    if (boxName != null)
    {
      if (boxName.equals(BOX_NAME_REPORT_SELECT))
      {
        ...
        {
          Query query = dataSession.createQuery("from TestConducted where latestTest = 'Y' and completeTest = 'Y' order by connectionLabel");
      ...
      } else
      {
        ...
        if (pentagonBox != null)
        {
          ...
          try
          {
            ...
            {
              out.println("<h3 class=\"pentagon\">Score History</h3>");
  ...
  public void printReportsRun(Session dataSession, PrintWriter out, List<TestConducted> testConductedList)
  {
    ...
    for (TestConducted testConductedDisplay : testConductedList)
    {
      ...
      out.println("<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"100\" height=\"100\">");
      {
        int offsetY = 0;
        int offsetX = 0;
        setupSmallPentagon(out, pentagonReportDisplay, offsetY, offsetX);
      }
      out.println("</svg>");
      out.println("    </td>");
